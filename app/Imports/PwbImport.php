...
class PwbImport implements ToModel, WithHeadingRow
{
    protected $rateFile;
...
//        dd($this->rateFile);
        ...
        foreach ($row as $key => $val) {
            ...
            if ($key != 'ages_terms') {
                ...
                $oldData = Pwb_rate::where('age_id', $record['age_id'])
                    ->where('term_id', $record['term_id'])
                    ->where('company_id', $record['company_id'])
                    ->where('product_id', $record['product_id'])
                    ->pluck('id');
                if ($oldData) {
                    $aaa = Pwb_rate::whereIn('id', $oldData)->update(['deleted_at' => Carbon::now()]);
                }
                if ($record['rate'] > 0){
                $dta = Pwb_rate::create([
