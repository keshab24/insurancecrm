...
@section('title')
    <p class="h4 align-center mb-0">Why Different Contents</p>
@endsection
@section('header_css')
...
@section('footer_js')
    <script type="text/javascript">
        var oTable = $('#whyDiffTable').dataTable();
        ...
        <div class="box-body">
            ...
            <div id="accordion">
              ...
              <i class="fa fa-angle-left pull-right"></i>
                    ...
                    <div id="collapseWhyDiff" class="collapse" aria-labelledby="WhyDiff" data-parent="#accordion">
                        <div class="card-body">
                            <form class="p-2" method="POST" action="{{route('why-different.update',1)}}"
                                  enctype="multipart/form-data">
                                {{method_field('patch')}}
                                {{csrf_field()}}
                                <div class="form-group">
                                    <label for="title">Left Title</label>
                                    <input type="text" name="why_diff_title"
                                           value="{{$whyContent['why_diff_title'] ? $whyContent['why_diff_title'] : ''}}"
                                           class="form-control"
                                           placeholder="Enter why us title">
                                </div>
                                <div class="form-group">
                                    <label for="why_diff_content">Left Description</label>
                                    <textarea name="why_diff_content" class="form-control" id="ckeditor" cols="5"
                                              rows="10">{{$whyContent['why_diff_content'] ? $whyContent['why_diff_content'] : ''}}</textarea>
                                </div>
            ...
            @if(count($why)< 4)
                <div class="justify-content-end list-group list-group-horizontal ">
                    ...
                    @include('Backend.WhyDifferent.add')
                </div>
            @endif
            <br>
            <div class="blank-page">
                <table class="table" id="whyDiffTable">
                    <thead>
                    <tr>
                        <th scope="col">#</th>
                    ...
                    @foreach($why as $why)
                        <tr id="row_{{ $why->id }}">
                            ...
                            <div class="modal fade" id="editModal{{$why['id']}}" tabindex="-1" role="dialog"
                                ...
                                @include('Backend.WhyDifferent.edit')
                            </div>
