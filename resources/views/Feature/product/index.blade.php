...
@endsection
@section('title')
    <p class="h4 align-center mb-0">Features Products</p>
@endsection
                        ...
                        @foreach($feature as $index=>$pro)
                            <tr class="gradeX" id="row_{{ $pro->id }}">
                                ...
                                <td class="name">
                                    @if(count($pro->products))
                                        @foreach($pro->products as $pro1)
                                            <ul>
                                                <li>{{ getProductName($pro1->product_id) }}</li>
                                            </ul>
                                        @endforeach
                                    @else
                                ...
                                <td class="justify-content-center">
                                    ...
                                    <a href='featureproList/{{  $pro->id }}' class="delete-discount"
                                       onclick="return confirm('Are you sure you want to Delete?');">
                                        <button
                                            type="button" class="btn btn-danger btn-flat"><i class="fa fa-trash"></i>
                                        </button>
                                    </a>
