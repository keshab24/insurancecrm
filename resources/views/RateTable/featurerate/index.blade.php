...
<script>
    $(document).ready(function () {
        ...
        $('.xedit').editable({
            url: '{{ url("admin/update-rate-table/feature-rate") }}',
            title: 'Update',
            ajaxOptions: {
                type: 'post'
            },
...
@endsection
@section('title')
<p class="h4 align-center mb-0">Feature Rate Table</p>
@endsection
...
<div class="box">
    ...
    <div class="box-body">
        ...
        <button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#collapseBulkImport"
            ...
            Click To Import Feature Rate Data
        </button>
        <div class="collapse" id="collapseBulkImport">
            ...
            <div class="card-body">
                ...
                {{-- <a href="{{ route('endowment.rates.export') }}"
                download>--}}
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6"><strong>Upload Feature Rate File</strong></h5>
                </div>
                <div class="card-body">
                    <form class="" action="{{ route('feature.rates.import') }}" method="POST"
                        ...
                        <div class="form-inline">
                            <div class="form-group mx-sm-12 mr-lg-3">
                                ...
                                <select class="form-control form-control-inline input-medium search-select"
                                    onchange="findProducts()" id="company1" name="feature_id" required>
                                    <option selected disabled>Select the Feature</option>
                                    @foreach($features as $feature)
                                        <option
                                            data-products="{{ getProductList($feature->products->pluck('product_id')) }}"
                            ...
                            <div class="form-group mx-sm-6">
                                ...
                                <select class="form-control form-control-inline input-medium search-select"
                                    id="products1" name="product_id" required>
            ...
            <div class="alert"
                ...
                <p>
                    3.
                    {{ ('Once you have downloaded and filled the skeleton file, upload it in the form and submit') }}
                    .</p>
            </div>
            <br>
        </div>
        <br>
        <hr>
        <form action="{{ route('rate.feature.index') }}" method="get" class="form-inline">
            {!! csrf_field() !!}
            <div class="">
                <div class="form-group mx-sm-3">
                    <label class="">Feature Name</label>
                    <select class="form-control form-control-inline input-medium search-select" onchange="findProduct()"
                        id="company" name="feature_id" required>
                        <option selected disabled>Select the Feature</option>
                        @foreach($features as $feature)
                            <option
                                data-products="{{ getProductList($feature->products->pluck('product_id')) }}"
        ...
        <span class="ml-5">[Please select Feature and product to View data.]</span><br>
        <div class="mx-auto text-center">
            @if(isset($selectedCompany))
                <label class=" mb-2">Selected Feature:
                    {{ $selectedCompany ? $selectedCompany->name : 'N/A' }}</label>
                <br>
                <label class=" mb-2">Selected Product:
                    {{ $selectedProduct ? $selectedProduct->name : 'N/A' }}</label>
                <br>
                <a href="{{ route('company.feature.rates.export',(['feature_id'=>$selectedCompany->id, 'product_id'=>$selectedProduct->id])) }}"
                    download>
                    <button class="btn btn-info mr-2">Download Feature Rate Endowment</button>
                </a>
            @endif
        </div>
        @if($columns && $rows)
            <div style="overflow-x:auto;">
                ...
                @include('RateTable.pwb_rate.table')
            </div>
        @else
